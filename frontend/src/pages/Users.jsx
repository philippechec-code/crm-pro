...
export default function Users() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast(e.response?.data?.error || 'Erreur création', 'error');
    }
    setLoading(false);
  };
  ...
  const handleChangePwd = async () => {
    ...
    setLoading(true);
    try {
      await usersApi.update(pwdTarget.id, { password: pwdForm.newPassword });
      toast(`Mot de passe de ${pwdTarget.full_name || pwdTarget.email} mis à jour`, 'success');
      closePwdModal();
    } catch (e) {
      toast(e.response?.data?.error || 'Erreur modification mot de passe', 'error');
    }
    setLoading(false);
  };
  ...
  const handleDelete = async (u) => {
    ...
    try {
      ...
      toast(e.response?.data?.error || 'Erreur suppression', 'error');
    }
  };
                    ...
                    <tr key={u.id} style={{ opacity: u.active === false ? 0.5 : 1 }}>
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: 4 }}>
                          <button className="btn btn-ghost btn-icon btn-sm" title="Changer le mot de passe" onClick={() => openPwdModal(u)}>
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="14" height="14">
                              <rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>
                            </svg>
                          </button>
                          <button className="btn btn-ghost btn-icon btn-sm" title={u.active !== false ? 'Désactiver' : 'Activer'} onClick={() => toggleActive(u)} disabled={isMe}>
                            {u.active !== false
                          ...
                          <button className="btn btn-danger btn-icon btn-sm" title="Supprimer" onClick={() => handleDelete(u)} disabled={isMe}>
          ...
          <div className="modal modal-sm" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                <div style={{ position: 'relative' }}>
                  <input className="form-control" type={pwdForm.show ? 'text' : 'password'}
                    value={pwdForm.newPassword} onChange={e => setPwdForm(f => ({ ...f, newPassword: e.target.value }))}
                    placeholder="Minimum 8 caractères" autoFocus style={{ paddingRight: 42 }}
              ...
              <div className="form-group" style={{ marginBottom: 0 }}>
                ...
                <input className="form-control" type={pwdForm.show ? 'text' : 'password'}
                  ...
                  placeholder="Répéter le mot de passe" onKeyDown={e => e.key === 'Enter' && handleChangePwd()} />
              </div>
            </div>
            <div className="modal-footer">
              <button className="btn btn-secondary" onClick={closePwdModal}>Annuler</button>
              <button className="btn btn-primary" onClick={handleChangePwd}
                disabled={loading || pwdForm.newPassword.length < 8 || pwdForm.newPassword !== pwdForm.confirm}>
